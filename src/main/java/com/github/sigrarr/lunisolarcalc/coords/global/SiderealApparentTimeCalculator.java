...
/**
 * Calculator of the {@linkplain GlobalCoord#SIDEREAL_APPARENT_TIME apparent sidereal time at the Greenwich meridian (θ0)}.
...
public class SiderealApparentTimeCalculator implements Provider<GlobalCoord, TimelinePoint> {
    ...
    public static final GlobalCoord SUBJECT = GlobalCoord.SIDEREAL_APPARENT_TIME;

    /**
     * Calculates the {@linkplain GlobalCoord#SIDEREAL_APPARENT_TIME apparent sidereal time at the Greenwich meridian (θ0)}
     * expressed in degrees: [0, 360°). 15° corresponds to 1 hour.
     *
     * @param meanSiderealTimeDegrees   {@linkplain GlobalCoord#SIDEREAL_MEAN_TIME mean sidereal time (θ0)}, in degrees
     ...
     * @return                          {@linkplain GlobalCoord#SIDEREAL_APPARENT_TIME apparent sidereal time at the Greenwich meridian (θ0)},
     *                                  in degrees: [0, 360°)
     */
    ...
    public EnumSet<GlobalCoord> requires() {
        return EnumSet.of(
            GlobalCoord.SIDEREAL_MEAN_TIME,
    ...
    public Double calculate(TimelinePoint tx, Map<GlobalCoord, Object> precalculatedValues) {
        return calculate(
            (Double) precalculatedValues.get(GlobalCoord.SIDEREAL_MEAN_TIME),
